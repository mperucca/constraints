...
sealed trait Fraction:
  val numerator: Int
  val denominator: Int
  given nonZeroDenominatorProof: Proof[denominator.type !== 0]
...
object Fraction:
  ...
  class WhiteBox[+N <: Int, +D <: Int](val numerator: N, val denominator: D)(
    using val nonZeroDenominatorProof: Proof[denominator.type !== 0]
  ) extends Fraction

  def apply(numerator: Int, denominator: Int)(using Proof[denominator.type !== 0]): Fraction.WhiteBox[numerator.type, denominator.type] =
    new Fraction.WhiteBox(numerator, denominator)
